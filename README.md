# 注意：このリポジトリにはサイトが2つ入っています

| 場所 | 内容 |
| --- | --- |
| このフォルダの `index.html` | **別件**のハッピーペイントホーム（雨漏り修理） |
| **`stern.zip`** | **ステルン（不用品回収）の WordPress テーマ** ← WordPressに入れるのはこれ |

ステルンをWordPressに入れる手順は `STERN_ここを読んでください.txt` を見てください。
